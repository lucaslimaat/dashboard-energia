...
// --- API CLIENTS ---
// Supabase
const supabaseUrl = process.env.SUPABASE_URL || "https://lpgohuagjekitxhmxkwt.supabase.co";
const supabaseAnonKey = process.env.SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxwZ29odWFnamVraXR4aG14a3d0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTMxMjg4MDMsImV4cCI6MjA2ODcwNDgwM30.psu2TDq1VJYcyyege8hUu1-84Iq3jLdDJZYZPV2JJlQ";
...
/**
 * Processes uploaded files with Gemini via a serverless function and saves them to Supabase.
 */
async function processFiles() {
  ...
  loadingText.textContent = 'Enviando arquivos para processamento...';
  ...
  try {
    ...
    if (!apiResponse.ok) {
      let errorMsg = `Erro do Servidor: ${apiResponse.status} ${apiResponse.statusText}`;
      try {
    ...
    const extractedData: Partial<GeminiBillData>[] = await apiResponse.json();
    loadingText.textContent = 'Salvando dados no banco de dados...';
    ...
    if (Array.isArray(extractedData)) {
      ...
      for (const billData of extractedData) {
        if (!billData.date || !billData.installationNumber) {
          console.warn("Conta ignorada: data de referência ou número de instalação não encontrados.", billData);
          ignoredCount++;
          continue;
        }
      ...
      let alertMessage = '';
      if (addedCount > 0) alertMessage += `${addedCount} conta(s) foi(ram) adicionada(s) com sucesso.\n`;
      if (duplicateCount > 0) alertMessage += `${duplicateCount} conta(s) duplicada(s) foi(ram) ignorada(s).\n`;
      if (ignoredCount > 0) alertMessage += `${ignoredCount} conta(s) foi(ram) ignorada(s) por falta de dados.\n`;
      if (alertMessage) await showCustomModal('Processamento Concluído', alertMessage.trim());
    ...
    console.error("Erro ao processar arquivos ou salvar no DB:", error);
    const errorMessage = error.message || 'Ocorreu um erro desconhecido.';
    let friendlyMessage = `Ocorreu um erro ao processar os arquivos.\n\nMotivo: ${errorMessage}`;
    
    if (errorMessage.includes('security policy')) {
        friendlyMessage += '\n\nIsso geralmente indica que as permissões (RLS) no Supabase não estão configuradas corretamente para a ação de "INSERT".'
    }

    await showCustomModal('Erro Inesperado', friendlyMessage);
  } finally {
